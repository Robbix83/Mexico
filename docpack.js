...
async function buildContext(pack, files, opts = {}) {
  ...
  return {
    ...
    submit_date:      data.submit_date ? (() => { try { const d=new Date(data.submit_date); return isNaN(d)?data.submit_date:d.toLocaleDateString('he-IL'); } catch{return data.submit_date;} })() : '',
    update_date:      data.update_date ? (() => { try { const d=new Date(data.update_date); return isNaN(d)?data.update_date:d.toLocaleDateString('he-IL'); } catch{return data.update_date;} })() : '',
